...
use miniq_protocol::{ImageAttachment, QueuedMessage};
...
impl Store {
    ...
    pub fn enqueue_message(
        ...
        conn.execute(
            "INSERT INTO queued_messages (id, session_id, content, images_json, position, created_at)
             VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
            params![
                message.id,
                message.session_id,
    ...
    pub fn list_queued_messages(&self, session_id: &str) -> Result<Vec<QueuedMessage>> {
        ...
        let mut stmt = conn.prepare(
            "SELECT id, session_id, content, images_json, position, created_at
...
mod tests {
    ...
    fn queued_images_roundtrip() {
        ...
        store
            .enqueue_message(&session_id, "inspect", &images)
            .unwrap();
        let queued = store.dequeue_message(&session_id).unwrap().unwrap();
