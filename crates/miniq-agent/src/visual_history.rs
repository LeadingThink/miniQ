    ...
    fn project_images(
        ...
        if !references.is_empty() {
            let metadata = json!({
                ...
                "note":"Image references and source metadata are retained locally. Original pixels remain readable while their local files are available. An included=false image is archived or already included elsewhere in this request. Explicit missing_visual_evidence or unavailable_attachment notices override inclusion metadata: unavailable pixels were not inspected in this request. Use image_history to read any needed reference before making a new visual judgment."
            });
