...
struct ResponsesDecoder {
    ...
    computer_observation: Option<String>,
}
...
impl ResponsesDecoder {
    ...
    fn update_call_from_item(&mut self, index: usize, item: &Value) {
        ...
        match kind {
            ...
            "computer_call" => {
                ...
                if item.get("pending_safety_checks").is_some_and(|checks| {
                    !checks.is_null() && checks.as_array().is_none_or(|checks| !checks.is_empty())
                }) {
                    call.blocked_reason = Some(
                        "computer_call requires confirmation of pending safety checks before execution",
                    );
                }
                let mut arguments = crate::responses_computer::arguments(item);
                if let Some(id) = &self.computer_observation {
                    arguments["observationId"] = json!(id);
                }
                call.arguments = arguments.to_string();
            }
            _ => {
    ...
    fn emit_call(&mut self, index: usize) -> Option<Result<ChatDelta, ProviderError>> {
        ...
        if let Some(reason) = call.blocked_reason {
            return Some(Err(ProviderError::InvalidResponse(reason.into())));
        }
...
impl ModelProvider for ResponsesProvider {
    ...
    async fn stream_complete(
        ...
        Ok(sse::response_stream(response, decoder))
    }
