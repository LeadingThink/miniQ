...
export function observationImage(call: ToolCall): ObservationImage | null {
  if (!["computer_use", "browser_automation"].includes(call.toolName)) return null;
  const value = (call.output as { screenshot?: Partial<ObservationImage> } | null)?.screenshot;
...
export async function loadObservation(client: RpcClient, call: ToolCall, signal: AbortSignal): Promise<Blob> {
