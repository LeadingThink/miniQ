...
import { closeBrowser, currentBrowser, openBrowser, setBrowserVisible } from "../browserWorkbench";
...
it("ignores stale native polls after a navigation", async () => {
  ...
  const ref = surface();
  const { result } = renderHook(() => useBrowserPanel("https://old.test/", ref));
  await act(async () => {});
...
it("hides the native child behind settings without discarding the page", async () => {
  const ref = surface();
  const { result, rerender } = renderHook(({ suspended }) => useBrowserPanel("https://example.test/", ref, suspended), {
