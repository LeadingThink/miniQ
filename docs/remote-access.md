...
- 桌面无需公网 IP 或端口映射。daemon 主动连接 relay，断线后指数退避重连；relay 短暂掉线时会保留手机连接 15 秒，给桌面恢复时间。

桌面端的本地聊天、文件、任务和审批由本机 daemon 提供，不依赖 relay。relay 只负责启用远程桌面时的加密转发；它不可用时，桌面本地功能仍可继续使用。
...
三个值相互独立，relay 不接收 API Key 原文，也不能解密 RPC、会话、工具参数或结果。房间在桌面在线时存在；桌面发生短暂网络抖动时，relay 会保留移动连接最多 15 秒，桌面恢复后继续使用原会话。超过宽限期仍未恢复时，移动连接才会关闭并进入正常重连流程；认证 token 不持久化。

同一个 API Key 对应同一个远程房间，房间内只允许一台桌面设备处于活动状态。同一台桌面因网络波动重连时会替换自己的旧连接，手机连接保持不变；另一台桌面使用相同 Key 接入时会收到冲突提示并在后台等待，不会抢占或干扰当前桌面。需要同时远程控制多台桌面时，应为每台桌面使用不同的 API Key。
...
- 网页端 API Key 只进入当前浏览器的 `sessionStorage`（用户主动勾选记住时才持久化）；原生 iOS/Android 端保存在系统安全存储中。设备 ID 才会进入网页端 `localStorage`，用于在重连时保持设备身份。
